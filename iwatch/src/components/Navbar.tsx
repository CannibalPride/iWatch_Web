...
const LogsIconButton = (props: LogsIconButtonProps) => {
  const {
    badgeContainerPosition = {
      top: isWebInternal() ? -10 : 2,
      left: undefined,
  ...
  const {setUnreadLogsCount, unreadLogsCount} = useChatNotification();
  const {setLogsActive, setSelectedChatUserId} =
    useChatUIControl();
  const {sidePanel, setSidePanel} = useSidePanel();
  //commented for v1 release
  //const chatLabel = useString('chatLabel')();
  const chatLabel = 'Logs';
  const defaultTemplateValue = useButtonTemplate().buttonTemplateName;
  const {buttonTemplateName = defaultTemplateValue} = props;
  const isPanelActive = sidePanel === SidePanelType.Logs;
  const onPress = () => {
    if (isPanelActive) {
      setSidePanel(SidePanelType.None);
      setLogsActive(false);
  ...
  let btnTemplateProps: BtnTemplateInterface = {
    onPress: onPress,
    name: isPanelActive ? 'eye_full' : 'eye_base',
  };
  if (buttonTemplateName === ButtonTemplateName.bottomBar) {
    btnTemplateProps.btnText = chatLabel;
    btnTemplateProps.style = Styles.localButtonWithoutBG as Object;
  ...
  return props?.render ? (
    props.render(onPress, isPanelActive, unreadLogsCount, buttonTemplateName)
  ) : (
    <>
      <BtnTemplate {...btnTemplateProps} />
      {sidePanel !== SidePanelType.Chat &&
        unreadLogsCount !== 0 &&
        renderBadge(unreadLogsCount)}
    </>
  );
};
