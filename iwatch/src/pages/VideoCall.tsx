...
import { iWatchVideoExtension } from "./video-extension";
import MonitorProvider, { MonitorContext } from '../components/MonitorContext';
...
const extension = new iWatchVideoExtension();
...
const VideoCall: React.FC = () => {
  const { isMonitoring, toggleProcessor, isGlobalMonitoring } = useContext(MonitorContext);
  ...
  const setupMonitoring = async () => {
    denoiser = denoiser || (() => {
      let denoiser = new iWatchVideoExtension();
    ...
    if (!localTracks.videoTrack && window.engine) {
      localTracks.videoTrack = window.engine.localStream.video;
    }
  }
  ...
  const startMonitoring = async () => {
    ...
    try {
      ...
      toggleProcessor(window.iwatchuid);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const cleanupMonitoring = async () => {
    if(localTracks.videoTrack){
      localTracks.videoTrack.unpipe(processor);
    }
  };

  useEffect(() => {
    if (isMonitoring.includes(window.iwatchuid)) {
      console.log("Monitoring is active - setting up");
      startMonitoring();
    } else {
      console.log("Monitoring is not active - cleaning up");
      cleanupMonitoring();
    }

    // Cleanup function to run when component unmounts or before next effect
    return () => {
      if (isMonitoring.includes(window.iwatchuid)) {
        cleanupMonitoring();
      }
    };
  }, [isGlobalMonitoring]); // Only re-run the effect if isMonitoring changes
