...
export interface ChatNotificationInterface {
  ...
  setUnreadLogsCount: React.Dispatch<SetStateAction<number>>;
  unreadIndividualLogsCount: individualUnreadMessageCount;
  setUnreadIndividualLogsCount: React.Dispatch<
    SetStateAction<individualUnreadMessageCount>
  >;
}
...
const ChatNotificationProvider = (props: ChatNotificationProviderProps) => {
  ...
  const [unreadIndividualLogsCount, setUnreadIndividualLogsCount] =
    useState<individualUnreadMessageCount>({});
  ...
  useEffect(() => {
    let logsUnreadCount = 0;
    for (const key in unreadIndividualLogsCount) {
      logsUnreadCount =
      logsUnreadCount + unreadIndividualLogsCount[key];
    }
    setUnreadLogsCount(logsUnreadCount);
  }, [unreadIndividualLogsCount]);
